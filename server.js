...
io.on('connection', (socket) => {
    ...
    socket.on('find_partner', (data) => {
        ...
        // Remove user from queue if they are already there (prevent duplicates)
        ...
        for (let i = 0; i < waitingUsers.length; i++) {
            ...
            // Check Block List (Bi-directional)
            const userBlockedByPartner = potentialPartner.blocked && potentialPartner.blocked.includes(socket.userData ? socket.userData.uid : null); // Basic ID check
            ...
            // Note: Since we don't have persistent UIDs for guests, this relies on what's available. 
            // For guests, blocking might be session-based if UIDs are regenerated. 
            // Assuming socket.userData.uid is stable for the session/login.
        ...
        if (bestMatch) {
            ...
            waitingUsers.push({ socket, interests: userInterests, blocked: data.blocked || [] });
            console.log(`User ${socket.id} added to queue with interests: ${userInterests.join(', ')}. Queue size: ${waitingUsers.length}`);
        }
    });

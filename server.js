...
io.on('connection', (socket) => {
    ...
    socket.on('find_partner', async (data) => {
        ...
        // --- QUEUE MANAGEMENT (Priority for VIP) ---
        // Fast Matchmaking: VIPs go to front, Free to back
        const queueUser = { socket, dbUser: currentUser, blocked: data.blocked, interests: userInterests };
        if (isPremium) {
            waitingUsers.unshift(queueUser);
        } else {
            waitingUsers.push(queueUser);
        }

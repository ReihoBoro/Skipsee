...
interface LandingPageProps {
    onStart: (settings: { gender: string, identity: string, interests: string[], country: string, user: GoogleUser | null, isVip: boolean }) => void;
...
export default function LandingPage({ onStart, heroText, subheadline }: LandingPageProps) {
    ...
    const confirmStart = () => {
        ...
        onStart({
            ...
            country,
            user, // Pass Google User object
            isVip // Pass VIP status
        });
    };
    ...
    // Runtime Config Fetch (Render Support)
    const [clientId, setClientId] = useState(process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "YOUR_GOOGLE_CLIENT_ID_PLACEHOLDER");

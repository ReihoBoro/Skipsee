        ...
        <div className="fixed inset-0 bg-black z-50 overflow-hidden text-white font-outfit">
                ...
                <div className="pointer-events-auto flex items-center gap-4">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="w-10 h-10 rounded-full bg-white/10 border border-white/20 overflow-hidden hover:scale-105 transition-transform flex items-center justify-center"
                        >
                            {user?.picture ? (
                        ...
                        {/* Dropdown Menu */}
                        {showProfileMenu && (
                            <div className="absolute top-12 right-0 bg-zinc-900 border border-white/10 rounded-xl w-64 shadow-2xl p-4 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
                                {/* Header */}
                                <div className="flex items-center gap-3 pb-3 border-b border-white/10">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="font-bold text-sm truncate max-w-[140px]">{user?.name || "Guest User"}</span>
                                        <span className="text-[10px] text-white/50">{user ? "Google Account" : "Limited Access"}</span>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 rounded-lg p-3 border border-amber-500/30">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-xs font-bold text-amber-500 flex items-center gap-1">
                                            <i className="fas fa-crown"></i> VIP Status
                                        </span>
                                        {isVip ? (
                                            <span className="text-[10px] bg-amber-500 text-black px-2 py-0.5 rounded-full font-bold">ACTIVE</span>
                                        ) : (
                                            <span className="text-[10px] text-white/50">Inactive</span>
                                        )}
                                    </div>
                                    {!isVip && (
                                        <button
                                            onClick={() => window.open('https://buymeacoffee.com/Reiho45', '_blank')}
                                            className="w-full py-1.5 bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold rounded shadow-lg shadow-amber-500/20 transition-all"
                                        >
                                            Upgrade Now
                                        </button>
                                    )}
                                ...
                                <button
                                    ...
                                    className="w-full py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 text-xs font-bold rounded border border-red-500/20 transition-colors flex items-center justify-center gap-2"
                                >
                                    <i className="fas fa-sign-out-alt"></i> End Session & Logout
                                </button>
                            </div>
                        )}
                    </div>
            ...
            <div ref={constraintsRef} className="relative w-full h-full">
                ...
                <div className="w-full h-full bg-zinc-900 relative">
                    ...
                    {partner && !isSearching && (
                        <div className="absolute top-24 left-1/2 -translate-x-1/2 md:top-8 bg-black/20 backdrop-blur-xl px-5 py-2 rounded-full flex items-center gap-2 border border-white/10 z-40 shadow-lg">

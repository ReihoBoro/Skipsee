...
async function init() {
    ...
    try {
        if (typeof io !== 'undefined') {
            // Connect to port 3000 if we are on localhost but NOT on port 3000 (e.g. Live Server 5500)
            const socketUrl = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && window.location.port !== '3000'
                ? 'http://localhost:3000'
                : '/';
            socket = io(socketUrl);
        } else {
